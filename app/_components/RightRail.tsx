  ...
  return (
    <aside className="py-4 px-4 space-y-4">
      <div className="flex items-center gap-2 rounded-full border border-border bg-bg-elevated px-4 py-2 text-sm">
        <svg viewBox="0 0 24 24" className="h-4 w-4 shrink-0 text-text-muted" fill="currentColor" aria-hidden>
      ...
      <div className="rounded-2xl border border-border bg-bg-elevated overflow-hidden">
        <h2 className="px-4 pt-3 pb-2 font-bold text-lg text-text">Trends for you</h2>
        {TRENDS.map((t) => (
          <div
            key={t.tag}
      ...
      <div className="rounded-2xl border border-border bg-bg-elevated overflow-hidden">
        <h2 className="px-4 pt-3 pb-2 font-bold text-lg text-text">Who to follow</h2>
        {SUGGEST.map((u) => (
          <div key={u.handle} className="flex items-center gap-3 px-4 py-3 hover:bg-bg transition">
            <div
              style={{ width: 40, height: 40, minWidth: 40, backgroundColor: u.color, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 700, fontSize: 16 }}
            >
              {u.name.charAt(0)}
            </div>
            <div className="min-w-0 flex-1">
              <p className="font-bold text-sm text-text truncate flex items-center gap-1">
                {u.name}
                <svg viewBox="0 0 24 24" className="h-3.5 w-3.5 text-accent shrink-0" fill="currentColor" aria-label="Verified">
