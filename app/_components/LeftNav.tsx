          ...
          return (
            <li key={label}>
              {href ? (
                ...
                <button type="button" className="flex w-full text-text cursor-default" tabIndex={-1}>
                  {inner}
                </button>
              )}
            </li>
...
export function MobileBottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 flex h-14 items-center justify-around border-t border-border bg-bg md:hidden">
      <Link href={FEED_URL} aria-label="Home" className="flex h-10 w-10 items-center justify-center rounded-full text-text transition hover:bg-bg-elevated">
        <HomeIcon />
      </Link>
