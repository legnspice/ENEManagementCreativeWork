...
export function CommentComposer({ onAdd }: { onAdd: (body: string) => void }) {
  ...
  return (
    <div className="flex gap-3 px-4 py-3 border-b border-border">
      <Avatar handle="You" name="You" size={40} src="/avatars/unknown.jpg" />
  ...
  return (
    <div>
      ...
      {local.map((c) => (
        <div
          key={c.id}
          className="flex gap-3 px-4 py-3 border-b border-border bg-bg-elevated/40"
        >
          <Avatar
            handle="You"
            name="You"
            size={36}
            src="/avatars/unknown.jpg"
          />
          <div className="min-w-0 flex-1">
            <span className="text-sm font-bold text-text">@you</span>
            <p className="mt-0.5 text-sm text-text leading-relaxed whitespace-pre-wrap">
              {c.body}
            </p>
          </div>
        </div>
